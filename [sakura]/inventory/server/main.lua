    ...
    if otherInv.type == 'shop' then
        if item.type == 'item_weapon' then
            if xPlayer.getMoney() >= item.count then 
                ...
                TriggerClientEvent('inventory:notify', src, 'error', 'Cannot afford this weapon!')
            end
        elseif item.type == 'item_standard' then
            if Config.PlayerWeight then
                if xPlayer.canCarryItem(item.name, count) then
                    ...
                    TriggerClientEvent('inventory:notify', src, 'error', 'Dein Rucksack ist voll!')
                end
            else 
