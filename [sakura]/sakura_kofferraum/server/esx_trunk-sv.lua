...
AddEventHandler('esx_trunk:getItem', function(plate, type, item, count)
  ...
  TriggerEvent('oxince:logMessage', 'trunk', '**' .. GetPlayerName(_source) .. '** hat **' .. count .. 'x ' .. item .. '** aus seinem Kofferraum (``' .. plate .. '``) genommen.', true)
      ...
      for i=1, #coffre,1 do
        if coffre[i].name == item then
          if (coffre[i].count >= count and count > 0) then
            ...
            local xAmount = count + xItem.count
            if xAmount > xItem.limit then
                TriggerClientEvent('esx:showNotification', _source, "~r~Du kannst dieses Item maximal ~w~" .. xItem.limit .. "~r~ besitzen!")
                return
            end
...
AddEventHandler('esx_trunk:putItem', function(plate, type, item, count,max, owned)
  ...
  TriggerEvent('oxince:logMessage', 'trunk', '**' .. GetPlayerName(_source) .. '** hat **' .. count .. 'x ' .. item .. '** in seinen Kofferraum (``' .. plate .. '``) gelegt.', true)
