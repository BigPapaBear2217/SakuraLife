...
Citizen.CreateThread(function()
	local shown = false
	local inLocking = false
	local inUnlocked = false
	local inLocked = false
	while true do
		Citizen.Wait(0)
		...
		if not shown and inZone and inLocking then
			shown = true
			exports['okokTextUI']:Open('Schliesse ab', 'darkblue', 'left')
		elseif not shown and inZone and inUnlocked then
			shown = true
			exports['okokTextUI']:Open('Aufgeschlossen', 'darkgreen', 'left')
		elseif not shown and inZone and inLocked then
			shown = true
			exports['okokTextUI']:Open('Abgeschlossen', 'darkred', 'left')
...
RegisterKeyMapping('doorlock', "Türen auf/zu schließen", 'keyboard', 'e')
