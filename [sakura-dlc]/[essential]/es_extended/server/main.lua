	...
	TriggerEvent('es:getPlayerFromId', _source, function(player)
		...
		table.insert(tasks, function(cb)
			...
			table.insert(tasks2, function(cb2)
                ...
                MySQL.Async.fetchAll('SELECT job, job_grade, loadout, position, level, rp, gang, gangrank FROM users WHERE identifier = @identifier', {
					['@identifier'] = player.getIdentifier()
				}, function(result)
                    ...
                    --Jason_Fraktion
                    if result[1].gang and result[1].gang ~= '' then
                        userData.gang = tostring(result[1].gang)
                    else
                        print('[es_extended] [^3WARNING^7] Hat keine Gang daher geben wir ihm nun Ganglos! Fix dein Scheiß!.')
                        userData.gang = "Ganglos"
                    end

                    if result[1].gangrank and result[1].gangrank ~= '' then
                        userData.gangrank = tostring(result[1].gangrank)
                    else
                        print('[es_extended] [^3WARNING^7] Hat kein Gangrank daher geben wir ihm nun Rank 0! Fix dein Scheiß!.')
                        userData.gangrank = "0"
                    end       
                    --Jason_Fraktion
		...
		Async.parallel(tasks, function(results)
            -- Jason_Level
			local xPlayer = CreateExtendedPlayer(player, userData.accounts, userData.inventory, userData.job, userData.loadout, userData.playerName, userData.lastPosition, userData.level, userData.rp, userData.gang, userData.gangrank)
...
TriggerEvent("es:addGroup", "supporter", "user", function(group) end)
TriggerEvent("es:addGroup", "management", "user", function(group) end)
