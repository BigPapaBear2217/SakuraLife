-- Level, RP : Jason_Level
function CreateExtendedPlayer(player, accounts, inventory, job, loadout, name, lastPosition, level, rp, gang, gangrank)
	local self = {}
	...
	self.setGang = function(gang)
		self.updateGang(gang)
	end

	self.updateGang = function(gang)
		self.gang = tostring(gang)
		self.triggerEvent('esx:setGang', self.gang)
		TriggerEvent('esx:setGang', self.source, self.gang)
	end
	
	self.setGangrank = function(gangrank)
		self.updateGangrank(gangrank)
	end

	self.updateGangrank = function(gangrank)
		self.gangrank = tostring(gangrank)
		self.triggerEvent('esx:setGangrank', self.gangrank)
		TriggerEvent('esx:setGangrank', self.source, self.gangrank)
	end
    --Jason_Fraktion
