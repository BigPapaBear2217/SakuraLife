...
TriggerEvent("es:addGroup", "guide", "user", function(group) end)
...
AddEventHandler('playerConnecting', function(user, set)
	for k,v in ipairs(GetPlayerIdentifiers(source))do
		...
		if banData ~= false then
			set("Du wurdest gebannt für: " .. banData.reason .. "\nAblauf Datum: " .. (os.date("%c", banData.expire)))
			CancelEvent()
			break
		end
	end
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				TriggerEvent('es:canGroupTarget', user.getGroup(), target.getGroup(), function(canTarget)
					if canTarget then
						...
						table.remove(reason, 1)
						if(#reason == 0)then
							reason = "Du wurdest von diesem Server gebannt."
						else
...
-- Kicking
TriggerEvent('es:addGroupCommand', 'kick', "guide", function(source, args, user)
	if args[1] then
		if(tonumber(args[1]) and GetPlayerName(tonumber(args[1])))then
			local player = tonumber(args[1])
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				table.remove(reason, 1)
				if(#reason == 0)then
					reason = "Du wurdest von diesem Server gekickt!"
				else
                    reason = "Du wurdest Gekickt! Grund: " .. table.concat(reason, " ")
				end
