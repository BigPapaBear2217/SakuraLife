...
local function drawTitle()
    if menus[currentMenu] then
		...
		elseif menus[currentMenu].titleFont == "~sprite~" then
            ...
            if HasStreamedTextureDictLoaded("WS_commonmenu") then
                SetUiLayer(0)
                DrawSprite("WS_commonmenu", "interaction_bgd", x, y, menuWidth, titleHeight, 0.0, 255, 255, 255, 255, 0)
            end
...
local function drawDescription()
    if menus[currentMenu] then
        ...
        if HasStreamedTextureDictLoaded("WS_commonmenu") then
			...
			DrawSprite("WS_commonmenu", "Gradient_Bgd", x, y, menuWidth, descriptionHeight, 0.0, 255, 255, 255, 220, 0)
...
local function drawMenuBackground()
    if menus[currentMenu] then
        ...
        if HasStreamedTextureDictLoaded("WS_commonmenu") then
            SetUiLayer(0)
            DrawSprite("WS_commonmenu", "Gradient_Bgd", x, y, menuWidth, menuHeight, 0.0, 255, 255, 255, 255, 0)
        else
            RequestStreamedTextureDict("WS_commonmenu")
        end
...
local function drawArrows()
    ...
    if HasStreamedTextureDictLoaded("WS_commonmenu") then
        ...
        DrawSprite("WS_commonmenu", "shop_arrows_upanddown", x, y, 0.0265, 0.05, 0.0, 255, 255, 255, 255, 0)
    else
        RequestStreamedTextureDict("WS_commonmenu")
    end
...
local function drawButton(text, subText)
    ...
    if multiplier then
        ...
        if text ~= "!!separator!!" then
            if menus[currentMenu].currentOption == optionCount and HasStreamedTextureDictLoaded("WS_commonmenu") then
                SetUiLayer(1)
                DrawSprite("WS_commonmenu", "Gradient_Nav", x, y, menuWidth, buttonHeight, 0.0, 255, 255, 255, 255, 0)
            end
...
function JayMenu.CheckBox(text, bool, callback)
    ...
    if JayMenu.SpriteButton(text, "WS_commonmenu", sprite, focusSprite) then
        bool = not bool
