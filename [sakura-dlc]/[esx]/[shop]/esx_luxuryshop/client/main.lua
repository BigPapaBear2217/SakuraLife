...
Citizen.CreateThread(function()
	...
	ESX.TriggerServerCallback('esx_luxuryshop:getCategories', function(categories)
		Categories = categories
	end)

	ESX.TriggerServerCallback('esx_luxuryshop:getVehicles', function(vehicles)
		Vehicles = vehicles
	end)
end)
...
RegisterNetEvent('esx_luxuryshop:sendCategories')
AddEventHandler('esx_luxuryshop:sendCategories', function(categories)
	Categories = categories
end)

RegisterNetEvent('esx_luxuryshop:sendVehicles')
AddEventHandler('esx_luxuryshop:sendVehicles', function(vehicles)
	Vehicles = vehicles
end)
...
function ReturnVehicleProvider()
	ESX.TriggerServerCallback('esx_luxuryshop:getCommercialVehicles', function(vehicles)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'return_provider_menu', {
			...
			TriggerServerEvent('esx_luxuryshop:returnProvider', data.current.value)
...
function OpenShopMenu()
	...
	for i=1, #Vehicles, 1 do
		if IsModelInCdimage(GetHashKey(Vehicles[i].model)) then
			...
			print(('[esx_luxuryshop] [^3ERROR^7] Vehicle "%s" does not exist'):format(Vehicles[i].model))
		end
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data2.current.value == 'yes' then
				...
				ESX.TriggerServerCallback('esx_luxuryshop:buyVehicle', function(success)
					if success then
						IsInShopMenu = false
						menu2.close()
						menu.close()
...
AddEventHandler('esx_luxuryshop:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_luxuryshop:hasExitedMarker', function(zone)
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('esx_luxuryshop:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('esx_luxuryshop:hasExitedMarker', LastZone)
		end
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) then
				...
				elseif CurrentAction == 'resell_vehicle' then
					ESX.TriggerServerCallback('esx_luxuryshop:resellVehicle', function(vehicleSold)
						if vehicleSold then
