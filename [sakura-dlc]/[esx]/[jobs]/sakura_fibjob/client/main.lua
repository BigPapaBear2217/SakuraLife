...
local coordinate = {
	...
	{ 482.57, -995.26, 30.68, nil, 180.07, nil, 1581098148}, -- Waffenkammer
	{ 463.57, -996.73, 30.70, nil, 90.31, nil, 1581098148}, -- Umkleide
...
function OpenCloakroomMenu()
	...
	local elements = {
		...
		{label = _U('fib_wear'), uniform = grade}
	}
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('esx_fibjob:getOtherPlayerData', function(playerData)
...
AddEventHandler('esx_phone:cancelMessage', function(dispatchNumber)
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'fib' and ESX.PlayerData.job.name == dispatchNumber then
...
AddEventHandler('esx_fibjob:hasEnteredEntityZone', function(entity)
	...
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'fib' and IsPedOnFoot(playerPed) then
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'fib' then
		...
		if IsControlJustReleased(0, 167) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'fib' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'fib_actions') then
			if not Config.EnableESXService then
				OpenfibActionsMenu()
			elseif playerInService then
				OpenfibActionsMenu()
