...
RegisterCommand("jail", function(src, args, raw)
	...
	if xPlayer["job"]["name"] == "police" or xPlayer["job"]["name"] == "fib" or xPlayer["job"]["name"] == "army" then
			...
			if jailTime ~= nil then
				...
				if args[3] ~= nil then
					GetRPName(jailPlayer, function(Firstname, Lastname)
						TriggerClientEvent('chat:addMessage', -1, { args = { "JUDGE",  Firstname .. " " .. Lastname .. " ist nun für: " .. args[3] .. " im knast" }, color = { 249, 166, 0 } })
					end)
				end
			else
...
AddEventHandler("esx-qalle-jail:jailPlayer", function(targetSrc, jailTime, jailReason)
	...
	if xPlayer["job"]["name"] == "police" or xPlayer["job"]["name"] == "fib" or xPlayer["job"]["name"] == "army" then
        ...
        TriggerClientEvent("esx:showNotification", src, GetPlayerName(targetSrc) .. "  wurde für " .. jailTime .. " Minuten inhaftiert!")
    end
end)
...
AddEventHandler("esx-qalle-jail:unJailPlayer", function(targetIdentifier)
    ...
    local xPlayer1 = ESX.GetPlayerFromId(src)

    if xPlayer1["job"]["name"] == "police" or xPlayer1["job"]["name"] == "fib" or xPlayer1["job"]["name"] == "army" then
        if xPlayer ~= nil then
        ...
        TriggerClientEvent("esx:showNotification", src, xPlayer.name .. " wurde freigelassen!")
    end
end)
