...
function JailLogin()
	...
	ESX.ShowNotification("Du bist im Gefängnis eingeschlafen! Wir haben dich wieder inhaftiert.")
...
function UnJail()
	...
	ESX.ShowNotification("Du wurdest aus dem Gefängnis entlassen! Streng dich an, dass du nicht wieder hier landest.")
end
...
function InJail()
	...
	Citizen.CreateThread(function()
		...
		while jailTime > 0 do
			...
			ESX.ShowNotification("Du musst noch " .. jailTime .. " verbleibende Minuten im Gefängnis verbringen.")
...
function OpenJailMenu()
	ESX.UI.Menu.Open(
		...
		{
			title    = "Gefängnis Menü",
			align    = 'top-left',
			elements = {
				{ label = "Nähste Person Inhaftieren", value = "jail_closest_player" },
				{ label = "Inhaftierte Personen Verwalten", value = "unjail_player" }
			}
		}, 
	function(data, menu)
			...
			ESX.UI.Menu.Open(
          	...
          	function(data2, menu2)
            	...
            	if jailTime == nil then
              		ESX.ShowNotification("Bitte gebe eine vernünftige Zeit in Minuten an!")
            	else
              		menu2.close()
              		...
              		if closestPlayer == -1 or closestDistance > 3.0 then
						...
						function(data3, menu3)
						  	...
						  	if reason == nil then
								ESX.ShowNotification("Du musst einen Grund für die Inhaftierung angeben.")
						  	else
								menu3.close()
		...
		elseif action == "unjail_player" then
			...
			ESX.TriggerServerCallback("esx-qalle-jail:retrieveJailedPlayers", function(playerArray)
				...
				if #playerArray == 0 then
					ESX.ShowNotification("Dein Gefängnis ist leer.")
					return
				end

				for i = 1, #playerArray, 1 do
					table.insert(elements, {label = "Wärter: " .. playerArray[i].name .. " | Hafteinheiten: " .. playerArray[i].jailTime .. "m", value = playerArray[i].identifier })
				end
