...
RegisterNetEvent('esx_holdupjew:currentlyRobbing')
AddEventHandler('esx_holdupjew:currentlyRobbing', function(currentStore)
	holdingUp, store = true, currentStore
end)

RegisterNetEvent('esx_holdupjew:killBlip')
AddEventHandler('esx_holdupjew:killBlip', function()
	RemoveBlip(blipRobbery)
end)

RegisterNetEvent('esx_holdupjew:setBlip')
AddEventHandler('esx_holdupjew:setBlip', function(position)
...
RegisterNetEvent('esx_holdupjew:tooFar')
AddEventHandler('esx_holdupjew:tooFar', function()
	holdingUp, store = false, ''
	ESX.ShowNotification(_U('robbery_cancelled'))
end)

RegisterNetEvent('esx_holdupjew:robberyComplete')
AddEventHandler('esx_holdupjew:robberyComplete', function(award)
	holdingUp, store = false, ''
	ESX.ShowNotification(_U('robbery_complete', award))
end)

RegisterNetEvent('esx_holdupjew:startTimer')
AddEventHandler('esx_holdupjew:startTimer', function()
	...
	while true do
					...
					if distance < 0.5 then
						...
						if IsControlJustReleased(0, Keys['E']) then
							if IsPedArmed(PlayerPedId(), 4) then
								TriggerServerEvent('esx_holdupjew:robberyStarted', k)
							else
		...
		if holdingUp then
			...
			if Vdist(playerPos.x, playerPos.y, playerPos.z, storePos.x, storePos.y, storePos.z) > Config.MaxDistance then
				TriggerServerEvent('esx_holdupjew:tooFar', store)
			end
		end
	end
end)
