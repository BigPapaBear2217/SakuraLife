...
RegisterCommand("geisel",function()
	callTakeHostage()
end)
...
function callTakeHostage()
	...
	if not canTakeHostage then 
		TriggerEvent("ot-notify:notify", "#ff0000", "Geisel", 'Du benötigst eine Waffe!') 
	end
	...
	if not takeHostage.InProgress and canTakeHostage then			
		...
		if closestPlayer then
			...
			if targetSrc ~= -1 then
				...
				TriggerEvent("ot-notify:notify", "#ff0000", "Geisel", 'Keine Geisel in der Nähe!') 
			end
		else
			TriggerEvent("ot-notify:notify", "#ff0000", "Geisel", 'Keine Geisel in der Nähe!') 
		end
...
Citizen.CreateThread(function()
	while true do 
		if takeHostage.type == "agressor" then
			...
			drawNativeText("Drücke [G] zum Freilassen, [H] zum killen!")
